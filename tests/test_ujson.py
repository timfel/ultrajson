...
@pytest.mark.skipif(
    sys.implementation.name in ("pypy", "graalpy"), reason="PyPy & GraalPy use incompatible GC"
)
...
@pytest.mark.xfail(
    sys.implementation.name == "graalpy", reason="GraalPy bug, it fails the last example."
)
@pytest.mark.parametrize(
    "test_input, expected",
    [
...
@pytest.mark.skipif(
    sys.implementation.name == "graalpy", reason="GraalPy has an incompatible stub implementation of getrefcount"
)
@pytest.mark.parametrize("sort_keys", [False, True])
...
@pytest.mark.skipif(
    sys.implementation.name == "graalpy", reason="GraalPy does not support buffer protocol for non C-contiguous buffers"
)
